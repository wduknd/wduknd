...
class InvBlock(nn.Module):
    def __init__(self, subnet_constructor, channel_num, channel_split_num, d = 1, clamp=0.8):
        ...
        self.split_len1 = channel_split_num  # 2
...
class Decoder(nn.Module):
    def __init__(self,channels,num_experts,k):
        ...
        self.pre_fuse = nn.Sequential(InvBlock(HinResBlock, 5*channels, 2*channels),
                                         nn.Conv2d(5*channels,channels,1,1,0))
        self.num_experts = num_experts
    def forward(self,x):
        x = self.pre_fuse(x)
        cof = self.gate(x)  ### [N,num_experts]
        out = torch.zeros_like(x).to(x.device)
        for idx in range(self.num_experts):
            if cof[:,idx].any()==0:
                continue
