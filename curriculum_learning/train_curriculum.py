...
class Trainer(object):
    def __init__(self, opt):
        ...
        # 创建训练数据集，添加课程学习支持
        train_set = AdaptiveCurriculumDataSetLoader(
            dataset_dir=opt.dataset_dir, 
            dataset_name=opt.trainset, 
            patch_size=opt.patchSize, 
            mode='train', 
            img_norm_cfg=opt.img_norm_cfg,
            curriculum_learning=opt.curriculum_learning,
            curriculum_epoch_start=opt.curriculum_epoch_start,
            curriculum_pace=opt.curriculum_pace,
            total_epochs=opt.epochs,
            adaptive_curriculum=opt.adaptive_curriculum
        )
        ...
        """ self.train_loader = CurriculumDataSetLoader(
            dataset=train_set, 
        ...
        for dkey in args.testset:
            valset = AdaptiveCurriculumDataSetLoader(dataset_dir=opt.dataset_dir, dataset_name=dkey, patch_size=None, mode='val' if dkey == args.trainset else 'test', img_norm_cfg=opt.img_norm_cfg)
    ...
    def train(self, epoch):
        ...
        print('Train epoch {:03d} on {}, loss: {:.4f}, time elapsed {}:{}s.'.format(epoch, self.trainset, avg_train_loss, int((toc - tic) // 60), int(toc - tic) % 60))
    ...
    def test(self, epoch ):
       ...
       # 用于更新自适应课程学习的性能指标
        ...
        for dkey in self.val_loaders.keys():
            ...
            # 更新当前最佳指标（用于自适应课程学习）
            if dkey == self.trainset:  # 只使用训练集对应的验证性能
                current_best_metric = max(current_best_metric, eval_mIoU)
            ...
            # 更新自适应课程学习策略
        if (hasattr(self.train_dataset, 'update_curriculum_with_metrics') and 
            current_best_metric > 0 and len(self.train_loss_history) > 0):
            
            current_pace = self.train_dataset.update_curriculum_with_metrics(
                current_best_metric, 
                self.train_loss_history[-1]
            )
            
            if current_pace is not None:
                self.writer.add_scalar('curriculum_pace', current_pace, epoch)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--trainset", default='IRSTD-1K', help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea', 'IRDST-real'")
    parser.add_argument("--testset", default='IRSTD-1K', help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea', 'IRDST-real'")
    
    #parser.add_argument("--trainset", default='NUAA-SIRST', help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea', 'IRDST-real'")
    #parser.add_argument("--testset", default='NUAA-SIRST',  help="dataset_name: 'NUAA-SIRST', 'NUDT-SIRST', 'IRSTD-1K', 'SIRST3', 'NUDT-SIRST-Sea', 'IRDST-real'")
    ...
    parser.add_argument('--curriculum_learning', action='store_true', help='是否启用课程学习')
    parser.add_argument('--curriculum_epoch_start', type=int, default=10,help='开始课程学习的epoch')
    parser.add_argument('--curriculum_pace', type=float, default=0.05,help='课程学习进度控制参数')
     # 自适应课程学习参数
    parser.add_argument("--adaptive_curriculum", action='store_true', default=True,help="Enable adaptive curriculum learning pace")
    ...
    #for itrainset in ['NUAA-SIRST']:
    #for itrainset in ['IRSTD-1K']:
    #for itrainset in ['NUDT-SIRST' ]:    
    #for itrainset in ['IRSTD_Domain_Gamma','NUAA_Domain_Gamma']:
        ...
        if trainer.mode=='train':
            ...
            for epoch in range(trainer.epoch_state, args.epochs):
                ...
                if (epoch + 1) % args.test_freq == 0:
                    print('-----------------------')
                    current_metric = trainer.test(epoch)
